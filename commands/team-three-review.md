...
**Usage:**
- `/team_three_review` — Review current branch with all six agents
- `/team_three_review [context]` — Review with additional context: `$ARGUMENTS`

---

## Architecture

The main agent (Opus) handles all deterministic work once, then passes context to six review agents: three standard reviews and three critical (adversarial) reviews.
...
|-------|------|-------|---------|
| Claude | Standard | Opus | `claude -p "Read /tmp/team-review-standard.md and follow the instructions." --dangerously-skip-permissions` |
| Claude | Critical | Opus | `claude -p "Read /tmp/team-review-critical.md and follow the instructions." --dangerously-skip-permissions` |
| Codex | Standard | GPT-5 | `codex exec --full-auto --skip-git-repo-check "Read /tmp/team-review-standard.md and follow the instructions."` |
| Codex | Critical | GPT-5 | `codex exec --full-auto --skip-git-repo-check "Read /tmp/team-review-critical.md and follow the instructions."` |
| Gemini | Standard | Gemini 3 Pro | `gemini -m gemini-3-pro-preview --yolo "Read /tmp/team-review-standard.md and follow the instructions."` |
| Gemini | Critical | Gemini 3 Pro | `gemini -m gemini-3-pro-preview --yolo "Read /tmp/team-review-critical.md and follow the instructions."` |
...
Extract STORY_ID from branch name if present.
...
Write `/tmp/team-review-context.md` with all gathered information:
...
Concatenate the canonical review commands with the context document. This keeps `code-review.md` and `code-review-critical.md` as the single source of truth.

```bash
# Standard prompt = canonical command + context
cat ~/.claude/commands/code-review.md > /tmp/team-review-standard.md
echo -e "\n\n---\n\n**Additional Context:** $ARGUMENTS\n\n---\n\n# CONTEXT DOCUMENT\n" >> /tmp/team-review-standard.md
cat /tmp/team-review-context.md >> /tmp/team-review-standard.md

# Critical prompt = canonical command + context
cat ~/.claude/commands/code-review-critical.md > /tmp/team-review-critical.md
echo -e "\n\n---\n\n**Additional Context:** $ARGUMENTS\n\n---\n\n# CONTEXT DOCUMENT\n" >> /tmp/team-review-critical.md
cat /tmp/team-review-context.md >> /tmp/team-review-critical.md
```
...
**Claude (Standard):**
```bash
claude -p "Read /tmp/team-review-standard.md and follow the instructions." --dangerously-skip-permissions
```

**Claude (Critical):**
```bash
claude -p "Read /tmp/team-review-critical.md and follow the instructions." --dangerously-skip-permissions
```

**Codex (Standard):**
```bash
codex exec --full-auto --skip-git-repo-check "Read /tmp/team-review-standard.md and follow the instructions."
```

**Codex (Critical):**
```bash
codex exec --full-auto --skip-git-repo-check "Read /tmp/team-review-critical.md and follow the instructions."
```

**Gemini (Standard):**
```bash
gemini -m gemini-3-pro-preview --yolo "Read /tmp/team-review-standard.md and follow the instructions."
```

**Gemini (Critical):**
```bash
gemini -m gemini-3-pro-preview --yolo "Read /tmp/team-review-critical.md and follow the instructions."
```
